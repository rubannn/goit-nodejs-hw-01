...
- `make scan` — запускає аналіз директорії та показує статистику по файлах
- `make duplicates` — шукає дублікати файлів у вказаній директорії
- `make organize` — копіює файли з джерела у цільову папку та сортує їх по категоріях
- `make cleanup` — показує, які старі файли будуть видалені, без фактичного видалення
- `make cleanup-confirm` — реально видаляє файли старші за вказану кількість днів
...
| --- | --- |
| `make scan` | `make s` |
| `make duplicates` | `make d` |
| `make organize` | `make o` |
| `make cleanup` | `make c` |
| `make cleanup-confirm` | `make cc` |
